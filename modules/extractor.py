...
def results_to_dataframe(
    ...
    keep_flag: str = "contain_competence_result",
    keep_value: str = "yes",
) -> pd.DataFrame:
    rows = []
...
def _flatten_dict(data: dict[str, Any], prefix: str = "") -> dict[str, Any]:
    ...
    for key, value in data.items():
        ...
        if _is_date_data(value):
            flat[column] = _normalized_date(value)
            flat[f"{column}_supporting_text"] = value.get("supporting_text")
            continue
